...
### 2.5 场景流程图示例（财务系统月报存档）

> 目标：体现“举例场景”如何落到文档中心标准流程（模板 -> 校验/规则 -> 处理 -> 归档 -> 回传）。

```mermaid
flowchart TD
  A[财务系统生成 2026-03 月报 PDF] --> B[调用 C# SDK 初始化上传会话]
  B --> C[指定模板 finance.report.monthly]
  C --> D[上传文件并提交业务键 FIN-2026-03]
  D --> E[模板字段校验: 年/月/部门 必填]
  E --> F[规则校验: 同年月唯一性 SQL 检查]
  F --> G{校验通过?}
  G -- 否 --> H[返回失败原因: 重复或字段缺失]
  G -- 是 --> I[写入对象存储 + 创建文档版本]
  I --> J[触发异步任务链]
  J --> K[执行加密]
  K --> L[执行电子签章与验签]
  ...
  M --> N[按日期+业务类型+业务键归档]
  N --> O[回传归档结果到财务系统]
  O --> P[财务系统页面嵌入文档组件可预览/下载]
```

场景落地要点：

- 与 2.1 通用流程一致，差异仅在模板编码、规则脚本和处理策略。
- 财务月报场景中，唯一性规则建议以 `tenant_id + biz_type + 年 + 月` 为联合约束。
- 回传结果建议包含 `documentId`、`archiveRecordId`、`traceId`，便于业务系统留痕与追溯。
