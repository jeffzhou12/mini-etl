...
本文件汇总了模型设计、流程图、关键 API、分层架构、脚本引擎等内容。
...
| 方案 | 优势 | 劣势 | 适用场景 |
|------|------|------|---------|
| **Quartz.NET** | 功能强大、集群支持、持久化 | 配置复杂、依赖较重 | 企业级应用 |
| **Hangfire** | 简单易用、UI界面、持久化 | 依赖数据库、轻量任务 | 中小型项目 |
#### 推荐方案：Quartz.NET

Quartz.NET 是功能最完整的企业级调度框架，适合 ETL 平台的复杂调度需求。

#### 关键特性实现

1. **Cron 表达式支持**：`0 */5 * * * ?` 每5分钟执行
2. **依赖注入**：Job 中可以注入服务
3. **集群支持**：多实例部署时任务不重复执行
4. **持久化**：调度信息存储到数据库
5. **错误处理**：自动重试、失败通知
6. **手动触发**：支持 API 手动触发任务
7. **动态管理**：运行时添加/修改/删除任务
...
0 0 12 * * ?       每天中午12点执行
0 0 0 1 * ?        每月1号执行
0 0 9-17 * * ?     每天9点到17点每小时执行
0 0/30 8-17 * * ?  工作时间每30分钟执行
```

#### 替代方案：Hangfire

如果项目需要更简单的方案，可以使用 Hangfire：

Hangfire 提供了开箱即用的 Dashboard UI，方便查看任务执行情况。
